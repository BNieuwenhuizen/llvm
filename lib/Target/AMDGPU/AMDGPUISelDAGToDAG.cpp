...
private:
  ...
  void SelectBUFFER_ATOMIC_CMPSWAP(SDNode *N);
...
// This is here because there isn't a way to use the generated sub0_sub1 as the
// subreg index to EXTRACT_SUBREG in tablegen.
void AMDGPUDAGToDAGISel::SelectBUFFER_ATOMIC_CMPSWAP(SDNode *N) {
  MVT VT = N->getSimpleValueType(0);
  bool Is32 = (VT == MVT::i32);
  SDLoc SL(N);

  unsigned Opcode = 0;
  SDValue VSrc, VCmp, SRsrc, VIndex, Offset, SLC;
  VSrc = N->getOperand(1);
  VCmp = N->getOperand(2);
  SRsrc = N->getOperand(3);
  VIndex = N->getOperand(4);
  Offset = N->getOperand(5);
  SLC = N->getOperand(6);

  SLC = CurDAG->getTargetConstant(cast<ConstantSDNode>(SLC)->getZExtValue(), SL, MVT::i1);
  SDValue CombinedSrc;
  if (Is32) {
    SDValue SrcRegSeqArgs[] = {
      CurDAG->getTargetConstant(AMDGPU::VReg_64RegClassID, SL, MVT::i32),
      VSrc,
      CurDAG->getTargetConstant(AMDGPU::sub0, SL, MVT::i32),
      VCmp,
      CurDAG->getTargetConstant(AMDGPU::sub1, SL, MVT::i32),
    };
    CombinedSrc = SDValue(CurDAG->getMachineNode(AMDGPU::REG_SEQUENCE, SL, MVT::i64, SrcRegSeqArgs), 0);
  } else {
    SDValue Sub0 = CurDAG->getTargetConstant(AMDGPU::sub0, SL, MVT::i32);
    SDValue Sub1 = CurDAG->getTargetConstant(AMDGPU::sub1, SL, MVT::i32);
    SDNode *Lo0 = CurDAG->getMachineNode(TargetOpcode::EXTRACT_SUBREG,
                                         SL, MVT::i32, VSrc, Sub0);
    SDNode *Hi0 = CurDAG->getMachineNode(TargetOpcode::EXTRACT_SUBREG,
                                         SL, MVT::i32, VSrc, Sub1);

    SDNode *Lo1 = CurDAG->getMachineNode(TargetOpcode::EXTRACT_SUBREG,
                                         SL, MVT::i32, VCmp, Sub0);
    SDNode *Hi1 = CurDAG->getMachineNode(TargetOpcode::EXTRACT_SUBREG,
                                         SL, MVT::i32, VCmp, Sub1);
    SDValue SrcRegSeqArgs[] = {
      CurDAG->getTargetConstant(AMDGPU::VReg_128RegClassID, SL, MVT::i32),
      SDValue(Lo0, 0),
      CurDAG->getTargetConstant(AMDGPU::sub0, SL, MVT::i32),
      SDValue(Hi0, 0),
      CurDAG->getTargetConstant(AMDGPU::sub1, SL, MVT::i32),
      SDValue(Lo1, 0),
      CurDAG->getTargetConstant(AMDGPU::sub2, SL, MVT::i32),
      SDValue(Hi1, 0),
      CurDAG->getTargetConstant(AMDGPU::sub3, SL, MVT::i32),
    };
    CombinedSrc = SDValue(CurDAG->getMachineNode(AMDGPU::REG_SEQUENCE, SL, MVT::i128, SrcRegSeqArgs), 0);
  }

  bool IndexZero = isa<ConstantSDNode>(VIndex) &&
                   cast<ConstantSDNode>(VIndex)->getZExtValue() == 0;
  SDValue VOffset, SOffset, ImmOffset;
  SOffset = CurDAG->getTargetConstant(0, SL, MVT::i32);
  ImmOffset = CurDAG->getTargetConstant(0, SL, MVT::i16);
  std::vector<SDValue> args;
  if (SelectMUBUFIntrinsicOffset(Offset, SOffset, ImmOffset)) {
    if (IndexZero) {
      Opcode = Is32 ? AMDGPU::BUFFER_ATOMIC_CMPSWAP_OFFSET_RTN :
                      AMDGPU::BUFFER_ATOMIC_CMPSWAP_X2_OFFSET_RTN;
      args = {CombinedSrc, SRsrc, SOffset, ImmOffset, SLC};
    } else {
      Opcode = Is32 ? AMDGPU::BUFFER_ATOMIC_CMPSWAP_IDXEN_RTN :
                      AMDGPU::BUFFER_ATOMIC_CMPSWAP_X2_IDXEN_RTN;
      args = {CombinedSrc, VIndex, SRsrc, SOffset, ImmOffset, SLC};
    }
  } else {
    if (!SelectMUBUFIntrinsicVOffset(Offset, SOffset, ImmOffset, VOffset)) {
      fprintf(stderr, "could not select SelectMUBUFIntrinsicVOffset\n");
    }
    if (IndexZero) {
      Opcode = Is32 ? AMDGPU::BUFFER_ATOMIC_CMPSWAP_OFFEN_RTN :
                      AMDGPU::BUFFER_ATOMIC_CMPSWAP_X2_OFFEN_RTN;
      args = {CombinedSrc, VOffset, SRsrc, SOffset, ImmOffset, SLC};
    } else {
      Opcode = Is32 ? AMDGPU::BUFFER_ATOMIC_CMPSWAP_BOTHEN_RTN :
                      AMDGPU::BUFFER_ATOMIC_CMPSWAP_X2_BOTHEN_RTN;

      SDValue AddrRegSeqArgs[] = {
        CurDAG->getTargetConstant(AMDGPU::VReg_128RegClassID, SL, MVT::i32),
        VIndex,
        CurDAG->getTargetConstant(AMDGPU::sub0, SL, MVT::i32),
        VOffset,
      ...
      SDValue VAddr(CurDAG->getMachineNode(AMDGPU::REG_SEQUENCE, SL, MVT::i64, AddrRegSeqArgs), 0);
      args = {CombinedSrc, VAddr, SRsrc, SOffset, ImmOffset, SLC};
    }
  }

  MemSDNode *Mem = cast<MemSDNode>(N);
  args.push_back(Mem->getChain());

  MachineSDNode *CmpSwap = CurDAG->getMachineNode(Opcode, SL, Is32 ? MVT::i64: MVT::i128, MVT::Other, args);
